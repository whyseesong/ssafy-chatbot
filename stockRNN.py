...
def pridict_stock_price(company_id):
    ...
    def reverse_min_max_scaling(price, testPredict):
        ...
        testPredict_np = np.asarray(testPredict)
        #print("price : " + str(price))
        return (testPredict*(np.max(price,0)-np.min(price,0)+1e-7)) + np.min(price,0)
    ...
    file_name = company_id + "_" + date + '.csv'

    st = pd.read_csv(file_name)
    st2 = st.drop(st.columns[[0, 6]], axis=1)
    st2.to_csv('stock_prophet.csv', index=False)

    # Open,High,Low,Close,Volume
    xyz = np.loadtxt('stock_prophet.csv', delimiter=',')
    # 역순(날짜순 배열)
    xyz = xyz[::-1]
    # Normalize(정규화)
    xy = MinMaxScaler(xyz)
...
# print(pridict_stock_price("005930"))
